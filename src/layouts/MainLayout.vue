<template>
	<q-layout view="lHh Lpr lFf">
		<q-header class="netease-bg-color-red flex" elevated style="min-height: 60px;">
			<q-toolbar>
				<q-icon size="md">
					<svg t="1622393525034" class="icon" viewBox="0 0 1024 1024" version="1.1"
						xmlns="http://www.w3.org/2000/svg" p-id="2039" width="200" height="200">
						<path
				...
				<q-toolbar-title>
					{{ $t('netease.label.appTitle') }}
				</q-toolbar-title>

				<q-btn no-caps stretch flat
					class="q-px-lg crm-title-font text-subtitle2 text-weight-medium"
					:label="$t('发现音乐')"
					:class="mainTabs['discoverMusic']"
					:to="{ name: 'Home' }"
				>
				</q-btn>
		...
		<q-footer class="netease-bg-color-red flex row" elevated style="min-height: 60px;">
<!--			<q-toolbar>-->
<!--				<q-toolbar-title>Footer</q-toolbar-title>-->
<!--			</q-toolbar>-->
			<q-toolbar class="justify-center">
				<q-btn flat icon="skip_previous" />
				<q-btn flat icon="play_circle_filled" v-if="!playing" />
				<q-btn flat icon="pause" v-else />
...
export default {
	...
	computed: {
		tabs () {
			let tabs = {};
			tabs[this.$route.name] = 'text-primary';
			if (this.$route.matched.some(route => route.meta.menu)) {
				const parentRoute = this.$route.matched.find(route => route.meta.menu);
				const menu = parentRoute.meta.menu;
				if (menu instanceof Array) {
					menu.forEach(object => {
						tabs[object] = 'text-primary';
					});
				} else {
					tabs[menu] = 'text-primary';
				}
			}
			return tabs;
		},
		mainTabs () {
			let mainTabs = ['discoverMusic', 'Approval', 'Reports', 'Notify'];
			let tabs = {};
			mainTabs.forEach(tab => {
				tabs[tab] = this.tabs[tab];
				if (tabs[tab] === 'text-primary') tabs[tab] += ' menu-selected';
			});
			return tabs;
		}
	}
